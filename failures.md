...
**Gold:** maybe | **Predicted:** no | **Retrieval hit:** False

The gold context was not retrieved in the top-5 — the only such case in the dense configuration. The question is four words long and names nothing but the candidate diagnosis, so the embedder has almost no signal to match on: no anatomy, no symptom, no procedure, no study design. The five passages it returned are all broadly gastroenterological (eosinophilic esophagitis, IBS, coeliac follow-up) but none come from the right record. With so little in the query, cosine similarity ranks on generic topical proximity rather than on the specific study.

The hybrid configuration did not rescue this case either; it is one of that system's two misses (the other being PubID 24139705). BM25 has the same problem for the opposite reason — the query contributes only one content term, "Crohn", so there is little lexical evidence to fuse.

**What would fix it:** query expansion, so a short question is enriched with related clinical terms before retrieval, or a cross-encoder reranker that scores (query, passage) pairs jointly over a wider candidate pool rather than relying on independent embeddings of a four-word string.
